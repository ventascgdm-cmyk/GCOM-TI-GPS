...
function enviarWA(vId) {
    ...
    let addrText = v.ubicacion_manual_raw || "Buscando..."; 
...
window.borrarHorarioModal = function() {
    ...
    if(confirm(`¿Estás seguro de BORRAR el horario de ${titulo}?`)) {
        ...
        mostrarNotificacion(`Horario de ${titulo} eliminado.`); 
        try { bootstrap.Modal.getInstance(document.getElementById('modalEditHora')).hide(); } catch(e){}
    }
};
    ...
    if (uData && choferData) {
        ...
        if (tkObj && activeSIDs[tkObj.token]) {
            peticionWialon(tkObj.url, "resource/bind_unit_driver", { resourceId: choferData.rid, unitId: uData.id, driverId: choferData.id, time: 0, mode: 1 }, activeSIDs[tkObj.token].sid).then(res => {
                ...
                else { console.log(`Chofer ligado en Wialon: ${nombreChoferLimpio}`); mostrarNotificacion(`✅ Chofer asignado exitosamente en Wialon.`); }
            });
        }
    }
}
...
function desencadenarGeocoding() {
    Object.keys(viajesActivos).forEach(vId => {
        ...
        if(uData && uData.pos) {
            ...
            // 1. SIEMPRE PEDIR LA DIRECCIÓN (Incluso si el viaje no ha iniciado)
            ...
            // 2. LÓGICA DE ARRIBOS Y AUDITORÍA (Bloqueada si no hay salida)
            if (v.t_salida && !v.t_fin) {
...
function procesarFilaDirecciones() {
    ...
    // Si ya existe en caché, lo pintamos y no gastamos límite del servidor
    ...
    .then(r => r.json())
    .then(d => {
        let a = d.display_name || "Dirección no disponible"; 
    ...
    .catch(e => console.log("Nominatim límite alcanzado, reintentando luego..."))
    .finally(() => { 
        // Obligatorio: esperar 1 segundo antes de la siguiente petición
        setTimeout(() => { isGeocoding = false; }, 1000); 
    });
}
...
// --- ACTUALIZACIÓN DE LA LISTA DE USUARIOS EN EL MODAL ---
    ...
    try { 
        ...
        let promesas = configSistema.tokens.map(async (tk) => { 
            try { 
                ...
                // Añade esta validación justo después:
                if (reqU && reqU.error === 1) {
                    console.warn(`Sesión inválida detectada en ${tk.nombre}. Reautenticando...`);
                    
                    // 1. Destruimos el SID corrupto
                    delete activeSIDs[tk.token];
                    
                    // 2. Solicitamos un login nuevo con el token original
                    let nuevoLogin = await peticionWialon(tk.url, "token/login", {token: tk.token});
                    if (nuevoLogin && nuevoLogin.eid) {
